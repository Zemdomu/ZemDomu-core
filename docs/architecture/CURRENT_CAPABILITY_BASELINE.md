...
| Semantic inspection / MCP | CLI implemented; MCP missing | `src/inspection.ts`; `tests/cli-inspection.test.ts`; `zemdomu graph`; `zemdomu inspect` | Deterministic human output covers files, components, semantic nodes and facts, source locations, and explicit unknowns. Structured consumers use the public graph/page APIs; ZD-14 remains a future experiment. |

...
| Semantic inspection / MCP | CLI implemented; MCP assessed, not shipped | `src/inspection.ts`; `tests/cli-inspection.test.ts`; `spikes/zd-14-readonly-mcp.ts`; `docs/architecture/MCP_FEASIBILITY_AND_THREAT_MODEL.md` | Deterministic CLI output covers files, components, semantic nodes and facts, source locations, and explicit unknowns. ZD-14 conditionally approves a separate two-tool local read-only proof only after containment and resource gates; MCP remains outside release scope. |

    ...
    return {
        ...
        analyzePage(context) {
            if (!(0, page_utils_1.isResolvedPage)(context))
                return [];
            const headings = context.page.facts.filter((fact) => fact.kind === 'heading' && fact.value === 1 && (0, page_utils_1.isUnconditional)(fact));
            ...
            return headings.slice(1).flatMap((fact) => {
                ...
                const related = [
                    ...(repeatedInstance ? [] : [(0, page_utils_1.relatedForFact)(first, context, 'First unconditional <h1> on this composed page')]),

...
exports.SEMANTIC_GRAPH_SCHEMA_VERSION = void 0;
exports.validateSemanticGraph = validateSemanticGraph;
exports.assertValidSemanticGraph = assertValidSemanticGraph;
exports.SEMANTIC_GRAPH_SCHEMA_VERSION = "1.0";
/**
...
function validateSemanticGraph(graph) {
    ...
    const compositionEndpointIds = new Set([
        ...componentIds,
        ...renderNodeIds,
    ]);
    if (graph.schemaVersion !== exports.SEMANTIC_GRAPH_SCHEMA_VERSION) {
        issues.push({
            code: "schema-version",
            path: "schemaVersion",
            message: `Expected semantic graph schema ${exports.SEMANTIC_GRAPH_SCHEMA_VERSION}.`,
        });
    }
    const registerId = (id, at) => {
    ...
    graph.components.forEach((component, index) => registerId(component.id, `components[${index}].id`));
    graph.renderNodes.forEach((node, index) => registerId(node.id, `renderNodes[${index}].id`));
    graph.imports.forEach((edge, index) => registerId(edge.id, `imports[${index}].id`));
    graph.composition.forEach((edge, index) => registerId(edge.id, `composition[${index}].id`));
    graph.pageRoots.forEach((root, index) => registerId(root.id, `pageRoots[${index}].id`));
    const paths = new Map();
    graph.files.forEach((file, index) => {
        ...
        file.componentIds.forEach((id, componentIndex) => {
            requireReference(componentIds.has(id), `files[${index}].componentIds[${componentIndex}]`, id, issues);
    ...
    graph.components.forEach((component, index) => {
        requireReference(fileIds.has(component.fileId), `components[${index}].fileId`, component.fileId, issues);
        component.renderRoots.forEach((root, rootIndex) => {
            if (root.state === "resolved") {
                requireReference(compositionEndpointIds.has(root.id), `components[${index}].renderRoots[${rootIndex}]`, root.id, issues);
            }
        });
        if (component.semanticOutput.state === "known") {
            const evidence = component.semanticOutput.evidence;
            requireReference(renderNodeIds.has(evidence.renderNodeId), `components[${index}].semanticOutput.evidence.renderNodeId`, evidence.renderNodeId, issues);
            evidence.componentPath.forEach((id, pathIndex) => requireReference(componentIds.has(id), `components[${index}].semanticOutput.evidence.componentPath[${pathIndex}]`, id, issues));
            if (evidence.componentPath.length === 0 ||
            ...
            const renderNode = graph.renderNodes.find((candidate) => candidate.id === evidence.renderNodeId);
            if (renderNode &&
    ...
    graph.renderNodes.forEach((node, index) => {
        if (node.fileId) {
            requireReference(fileIds.has(node.fileId), `renderNodes[${index}].fileId`, node.fileId, issues);
        }
    ...
    graph.imports.forEach((edge, index) => {
        requireReference(fileIds.has(edge.sourceFileId), `imports[${index}].sourceFileId`, edge.sourceFileId, issues);
        if (edge.target.state === "resolved") {
            requireReference(fileIds.has(edge.target.id) || componentIds.has(edge.target.id), `imports[${index}].target`, edge.target.id, issues);
        }
    });
    graph.composition.forEach((edge, index) => {
        requireReference(compositionEndpointIds.has(edge.from), `composition[${index}].from`, edge.from, issues);
        if (edge.to.state === "resolved") {
            requireReference(compositionEndpointIds.has(edge.to.id), `composition[${index}].to`, edge.to.id, issues);
        }
        if (edge.order.state === "known" &&
        ...
        if (edge.traversal.state === "boundary") {
            if (edge.traversal.reason === "cycle") {
                if (edge.traversal.cycle.length < 2 ||
            ...
            else if (edge.traversal.depth < 0 ||
    ...
    graph.pageRoots.forEach((root, index) => {
        if (root.rootComponent.state === "resolved") {
            requireReference(componentIds.has(root.rootComponent.id), `pageRoots[${index}].rootComponent`, root.rootComponent.id, issues);
        }
        root.renderRoots.forEach((renderRoot, rootIndex) => {
            if (renderRoot.state === "resolved") {
                requireReference(compositionEndpointIds.has(renderRoot.id), `pageRoots[${index}].renderRoots[${rootIndex}]`, renderRoot.id, issues);
            }
        });
    });
    ...
    provenances.forEach(({ provenance, path }) => {
        if (provenance.fileId !== undefined) {
            requireReference(fileIds.has(provenance.fileId), `${path}.fileId`, provenance.fileId, issues);
        }
        validateRange(provenance.range, `${path}.range`, issues);
    });
    return issues;
}
function assertValidSemanticGraph(graph) {
    const issues = validateSemanticGraph(graph);
    if (issues.length === 0)
        return;
    throw new Error(`Invalid semantic graph:\n${issues
        .map((issue) => `- [${issue.code}] ${issue.path}: ${issue.message}`)
        .join("\n")}`);
}
function requireReference(exists, at, id, issues) {
    if (exists)
        return;
    issues.push({
...
function validateRange(range, at, issues) {
    if (!range)
        return;
    const positions = [
    ...
    if (range.end)
        positions.push([range.end, `${at}.end`]);
...
function collectProvenances(graph) {
    const result = [];
    const add = (provenance, path) => {
        result.push({ provenance, path });
    };
    const addUnknown = (unknown, path) => {
        add(unknown.provenance, `${path}.provenance`);
    };
    if (graph.boundary.completeness.state === "incomplete") {
        graph.boundary.completeness.unknowns.forEach((unknown, index) => addUnknown(unknown, `boundary.completeness.unknowns[${index}]`));
    }
    ...
    graph.components.forEach((entity, index) => {
        ...
        if (entity.semanticOutput.state === "known") {
            ...
            add(entity.semanticOutput.evidence.provenance, `components[${index}].semanticOutput.evidence.provenance`);
        }
        else {
    ...
    graph.composition.forEach((entity, index) => {
        ...
        else if (entity.condition.kind === "branch" &&
